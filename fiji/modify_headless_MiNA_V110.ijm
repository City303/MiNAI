...
var punctatesARRAY = newArray();
var rodsARRAY = newArray();
var networks_newARRAY = newArray();
var mean_rod_lenARRAY = newArray();
var mean_network_lenARRAY = newArray();
var mean_punctate_lenARRAY = newArray();
var total_meanARRAY = newArray();
...
macro "MiNA Single Image Action Tool - C059T3e16S" {
	...
	showMessage("MiNA: Producing test skeleton...");
	...
	showMessage("MiNA: Skeletonizing...");
	run("Skeletonize (2D/3D)");
	run("Red");
	selectWindow("Original");
	run("Add Image...", "image=TestSkeleton x=0 y=0 opacity=100 zero");
	showMessage("MiNA: Done skeletonizing.");
	...
	showMessage("MiNA: Processing skeleton...");
	selectWindow("TestSkeleton");
	...
	showMessage("Entering challenge #1.");
	selectWindow("Results"); 
	rows = nResults;
	
	BranchCounts = newArray(rows);
	AvgBranchLengths = newArray(rows);
	for (i=0; i<rows; i++) {
		BranchCounts[i] = getResult("# Branches", i);
		AvgBranchLengths[i] = getResult("Average Branch Length", i);
	}
	showMessage("Passed challenge #1.");

	punctates = 0; rods = 0; networks_new = 0; network_branch_count = 0;
	mean_rod_len = 0; mean_network_len = 0;
	mean_punctate_len = 0;
	//Check counts of branches...
	for(i = 0; i<BranchCounts.length; i++){
	      if(BranchCounts[i] == 0){
	      	punctates = punctates + 1;
		mean_punctate_len = mean_punctate_len + AvgBranchLengths[i];
	      }
	      else if(BranchCounts[i] == 1){
	      	rods = rods + 1;
		mean_rod_len = mean_rod_len + AvgBranchLengths[i];
	      }
	      else{
		networks_new = networks_new + 1;
		mean_network_len = mean_network_len + AvgBranchLengths[i]*BranchCounts[i];
		network_branch_count = network_branch_count + BranchCounts[i];
	      }	
	}
	//Sanity check reconstruct total mean:
	total_mean = (mean_rod_len + mean_network_len)/(rods + network_branch_count);
	//Separated means for rod, network, punctate:
	mean_rod_len = mean_rod_len / rods;
	mean_network_len = mean_network_len / network_branch_count;
	mean_punctate_len = mean_punctate_len / punctates;

	selectWindow("Results"); run("Close");
	
	//This information comes from Branch Information window in Analyze skeleton 2d/3d
	...
	// Compute mean, median, stdev of number of network branches
	meanBranches = mean(networkBranchCounts);
	medianBranches = median(networkBranchCounts);
	sdBranches = sd(networkBranchCounts);

	Measurement = newArray("Punctates",
				"Rods",
				"Networks (Aji was here)",
				"Mean Rod Length",
				"Mean Network Length",
				"Mean Punctate Length",
				"Reconstructed Mean Branch Length",
				"Individuals",
			       "Networks",
			       "Mean Branch Length",
...
macro "MiNA Batch Analysis Action Tool - C059T3e16B" {
    ...
    punctates = punctatesARRAY;
    rods = rodsARRAY;
    networks_new = networks_newARRAY;
    mean_rod_len = mean_rod_lenARRAY;
    mean_network_len = mean_network_lenARRAY;
    mean_punctate_len = mean_punctate_lenARRAY;
    total_mean = total_meanARRAY;
...
function processFile(path) {
	...
	punctatesARRAY = Array.concat(punctatesARRAY, punctates);
	rodsARRAY = Array.concat(rodsARRAY, rods);
	networks_newARRAY = Array.concat(networks_newARRAY, networks_new);
	mean_rod_lenARRAY = Array.concat(mean_rod_lenARRAY, mean_rod_len);
	mean_network_lenARRAY = Array.concat(mean_network_lenARRAY, mean_network_len);
	mean_punctate_lenARRAY = Array.concat(mean_punctate_lenARRAY, mean_punctate_len);
	total_meanARRAY = Array.concat(total_meanARRAY, total_mean);
        ...
        filenameARRAY = Array.concat(filenameARRAY, path);
	punctatesARRAY = Array.concat(punctatesARRAY, "");
	rodsARRAY = Array.concat(rodsARRAY, "");
	networks_newARRAY = Array.concat(networks_newARRAY, "");
	mean_rod_lenARRAY = Array.concat(mean_rod_lenARRAY, "");
	mean_network_lenARRAY = Array.concat(mean_network_lenARRAY, "");
	mean_punctate_lenARRAY = Array.concat(mean_punctate_lenARRAY, "");
	total_meanARRAY = Array.concat(total_meanARRAY, "");
